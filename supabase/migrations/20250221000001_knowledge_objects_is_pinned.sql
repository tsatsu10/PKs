-- Add is_pinned to knowledge_objects for favorites / quick access
ALTER TABLE public.knowledge_objects
  ADD COLUMN IF NOT EXISTS is_pinned BOOLEAN NOT NULL DEFAULT false;

CREATE INDEX IF NOT EXISTS idx_knowledge_objects_is_pinned
  ON public.knowledge_objects(user_id, is_pinned)
  WHERE is_pinned = true;

COMMENT ON COLUMN public.knowledge_objects.is_pinned IS 'When true, object appears at top of lists (pinned for quick access).';

-- Order search results with pinned first
