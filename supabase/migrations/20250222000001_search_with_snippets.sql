-- Search with snippets: RPC that returns same rows as search_knowledge_objects plus a snippet (ts_headline).
-- Use when search_query is non-empty to show match context in UI.
...
COMMENT ON FUNCTION public.search_knowledge_objects_with_snippets IS 'Search with optional snippet (ts_headline) for match context. Returns jsonb rows with all knowledge_object fields plus snippet.';

GRANT EXECUTE ON FUNCTION public.search_knowledge_objects_with_snippets TO authenticated;
GRANT EXECUTE ON FUNCTION public.search_knowledge_objects_with_snippets TO service_role;
